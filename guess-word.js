      ...
      if (index === inputFieldIndEl.length - 1 && e.target.value) {
        ...
        if (answerRender === input.answer) {
          ...
          if (gameUpdate.finalScore === guessWordArray.length) {
            ...
            finishGameBoxEl.innerHTML = `<p class='game-finish-paragraph'>Congratulations!</p> 
             <p class='game-finish-paragraph'>You completed ${gameUpdate.finalScore} out of ${guessWordArray.length} </p>
            <p class='game-finish-paragraph'>You have finished the game</p>
...
const endGameResult = function () {
  ...
  endGameBoxEl.innerHTML = `<p class='game-end-paragraph'>${
    percentScore >= 70 ? "Excellent" : "Fair"
  } performance</p> 
   <p class='game-end-paragraph'>You completed ${
     gameUpdate.finalScore
   } out of ${guessWordArray.length} (${percentScore}%) </p>
  <p class='game-end-paragraph'>Play to finish</p>
